...
const findInvalidCards = (nestedArr) => {
  ...
  for (let index = 0; index < nestedArr.length; index++) {
    if (validateCred(nestedArr[index]) === false) {
      invalidCardList.push(nestedArr[index]);
    }
  }
...
const idInvalidCardCompanies = (nestedArr) => {
  ...
  for (let idx = 0; idx < nestedArr.length; idx++) {
    if (
      nestedArr[idx][0] === 3 &&
      companies.includes("Amex (American Express)") === false
    ) {
      companies.push("Amex (American Express)");
    } else if (
      nestedArr[idx][0] === 4 &&
      companies.includes("Visa") === false
    ) {
      companies.push("Visa");
    } else if (
      nestedArr[idx][0] === 5 &&
      companies.includes("Mastercard") === false
    ) {
      companies.push("Mastercard");
    } else if (
      nestedArr[idx][0] === 6 &&
      companies.includes("Discover") === false
    ) {
...
let invalidCards = [];
invalidCards.push(invalid1, invalid2, invalid3);
